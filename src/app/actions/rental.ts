...
export async function createRental(data: {
    ...
    try {
        ...
        if (existingConflicts.length > 0) {
            throw new Error('One or more selected items are already rented during this period')
        }
        ...
        const timestamp = nowMs.getFullYear().toString() +
...
export async function updateRental(id: string, data: {
    ...
    try {
        ...
        const now = new Date()
        const isActive = new Date(data.startDate) <= now && new Date(data.endDate) >= now
        ...
        if (previousRental) {
            ...
            const removedItemIds = previousItemIds.filter((id: string) => !itemIds.includes(id))
...
export async function deleteRental(id: string) {
    try {
        ...
        const now = new Date()
        const isCurrentlyActive = new Date(rental.startDate) <= now && new Date(rental.endDate) >= now
